...
import { X } from 'src/libs/icons/X'
...
>(({ className, children, ...props }: { className?: string; children: React.ReactNode }, ref) => {
  return (
    <DialogPrimitive.Overlay
...
const DialogTitle = React.forwardRef<DialogPrimitive.TitleRef, DialogPrimitive.TitleProps>(
  ({ className, ...props }, ref) => (
    <DialogPrimitive.Title
      ref={ref}
      className={cn(
        'text-lg native:text-xl text-foreground font-lexend-bold leading-none tracking-tight',
        className,
      )}
      {...props}
    />
